...
TidyInputSource* TIDY_CALL tidyInputSourceCreate( TidyDoc tdoc ) /* [i_a] */
{
	TidyDocImpl* doc = tidyDocToImpl( tdoc );
	TidyInputSource* ret = (TidyInputSource*)TidyDocAlloc( doc, sizeof(TidyInputSource) );
    assert( ret != NULL );
	if ( ret )
	{
		TidyClearMemory( ret, sizeof(TidyInputSource) );
	}
	return ret;
}

/** Destroy data structure created with tidyInputSourceCreate(). */
void TIDY_CALL tidyInputSourceDestroy( TidyDoc tdoc, TidyInputSource* source ) /* [i_a] */
{
	TidyDocImpl* doc = tidyDocToImpl( tdoc );
	if ( source )
	{
		TidyClearMemory( source, sizeof(TidyInputSource) );
		TidyDocFree( doc, source );
	}
}

TidyOutputSink* TIDY_CALL tidyOutputSinkCreate( TidyDoc tdoc ) /* [i_a] */
{
	TidyDocImpl* doc = tidyDocToImpl( tdoc );
	TidyOutputSink* ret = (TidyOutputSink*)TidyDocAlloc( doc, sizeof(TidyOutputSink) );
    assert( ret != NULL );
	if ( ret )
	{
		TidyClearMemory( ret, sizeof(TidyOutputSink) );
	}
	return ret;
}

/** Destroy data structure created with tidyBufCreate(). */
void TIDY_CALL tidyOutputSinkDestroy( TidyDoc tdoc, TidyOutputSink* sink ) /* [i_a] */
{
	TidyDocImpl* doc = tidyDocToImpl( tdoc );
	if ( sink )
	{
		TidyClearMemory( sink, sizeof(TidyOutputSink) );
		TidyDocFree( doc, sink );
	}
}
...
size_t TIDY_CALL tidyTell( TidyInputSource* source )
{
	return source->tell( source->sourceData );
}
...
size_t TY_(ReadRawBytesFromStream)( StreamIn *in, byte* buf, size_t count )
{
    ...
    for ( ix = 0; ix < count; ++ix )
    {
        ...
        buf[ix] = (byte)in->source.getByte( in->source.sourceData );
#else
		int c = in->source.getByte( in->source.sourceData );
		if (c == EOF)
			break;
		buf[ix] = (byte)c;
#endif
...
#if 0 /* [i_a] reduce overhead; ReadByte() et al return EndOfStream by themselves amyway */
	if ( TY_(IsEOF)(in) )
        return EndOfStream;
#endif
...
ctmbstr TY_(GetEncodingNameFromTidyId)(uint id)
{
    ...
    for (i = 0; i < sizeof(enc2iana)/sizeof(enc2iana[0]); ++i) /* [i_a] */
        if (enc2iana[i].id == id)
